...
public class StatusListAdapter extends AbstractAppListAdapter<MessageBean> {
	...
	protected void bindViewData(final ViewHolder holder, int position) {
		...
		if (drawable != null) {
			holder.listview_root.setBackground(drawable);
		} else {
		...
		if (showOriStatus) {
			...
			boolean checkPic = (!TextUtils.isEmpty(msg.getThumbnail_pic()) || (msg
					.getRetweeted_status() != null && !TextUtils.isEmpty(msg
					.getRetweeted_status().getThumbnail_pic())));
			checkPic = (checkPic && !SettingUtility.isEnableFilter());
